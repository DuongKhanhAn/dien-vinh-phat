...
router.post('/products', async (req, res) => {
  try {
    const { name, slug, brand, price, stock=0, min_stock=5, weight_kg=1, description, image_url, category_id, specs={}, upsell_ids=[], is_active=true } = req.body;
    if (!name || !slug || !price || !category_id) return res.status(400).json({ error: 'Thiếu thông tin bắt buộc' });
    const p = await Product.create({ name, slug, brand, price, stock, min_stock, weight_kg, description, image_url, category: category_id, specs, upsell_ids, is_active });
    res.status(201).json({ message: 'Thêm thành công', id: p._id });
  } catch (err) { res.status(err.code === 11000 ? 409 : 500).json({ error: err.code === 11000 ? 'Slug đã tồn tại' : err.message }); }
});

router.patch('/products/:id', async (req, res) => {
  try {
    const { category_id, specs, upsell_ids, ...rest } = req.body;
    const update = { ...rest };
    if (category_id) update.category = category_id;
    if (specs) update.specs = specs;
    if (upsell_ids) update.upsell_ids = upsell_ids;
