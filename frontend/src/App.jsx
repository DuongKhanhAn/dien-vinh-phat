    ...
    <footer style={styles.footer}>
      <div className="container">
        ...
        <div style={styles.footerBottom}>
          <p>© 2026 {SHOP.name}. Bảo lưu mọi quyền.</p>
        </div>
      </div>
    </footer>
  );
...
function AdminCouponsPage() {
  ...
  const createCoupon = async (e) => {
    e.preventDefault();
    if (!form.code.trim()) return toast('Vui lòng nhập mã giảm giá', 'error');
    if (!form.type) return toast('Vui lòng chọn loại mã', 'error');
    if (form.type !== 'free_ship' && (!form.value || parseFloat(form.value) <= 0)) {
      return toast('Vui lòng nhập giá trị giảm giá (số > 0)', 'error');
    }
    try {
      const payload = {
        code: form.code.trim().toUpperCase(),
        type: form.type,
        value: form.type === 'free_ship' ? 0 : parseFloat(form.value),
